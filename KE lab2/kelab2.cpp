...
int main(){
    ifstream in1("kelab_bruteforce_input.txt");
    string str ;
    string cur_trans;
    map<string,int> mp;
 ...
 /*
    ...
    */
    cout << "Items Scanned Along With Their Freq. >= threshold \n";
    vector<string> v;
    ...
    */
    cout << "Final list of most frequent subsets is : \n";
    ...
    for( it1=temp_ans.begin();it1!=temp_ans.end();it1++){
        if(it1->second >= threshold){
            ...
            for(int j=0;j<it1->first.size();j++){
                cout << it1->first[j] ;
                if(j!=it1->first.size()-1) cout << ", " ;
            }
            cout << " } (freq : " << it1->second << ")\n";
        }
    }

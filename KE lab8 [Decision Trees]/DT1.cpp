...
struct Dtree{
    string data ;
    bool isClass = false ;
    int cnt = 0 ; // count of cilhds pointers
    string condition[50] ;
...
void createDecisionTree(dtptr &root,map<string,vector<string> > records,int NOC,string className)
{
    ...
    {
        if(tempMap1[records[selectedColumn][i]] ==false)
        {
            vecStr.push_back(records[selectedColumn][i]) ;
    ...
    for(int it=0;it<vecStr.size();it++)
    {
        //string cond = it->first ;
        string cond = vecStr[it] ;
        map<string,vector<string> > tempRecord ;
        
        for(int i=0;i<records[selectedColumn].size();i++)
        {
            if(records[selectedColumn][i] == cond)
            {
        ...
        /*if(cond == "strong")
        {
            ...
            {
                cout<<it4->first<<" --  ";
                for(int ij=0;ij<it4->second.size();ij++)
                {
                    cout<<it4->second[ij]<<"   ";
                }
                cout<<endl ;
...
void printPaths(dtptr root,string str[],int i)
{
    ...
    // if(root)
    // {
    //     if(root->isClass == false)
    //     {
    //         cout<<root->data<<"------------";
    //         for(int j=0;j<root->cnt ;j++)
    //         {
    //             cout<<root->condition[j]<<"   " ;
    //         }
    //     }
    //     cout<<endl;
    //     for(int i=0;i<root->cnt ;i++)
    //     {
    //         printPaths(root->child[i],str,i) ;
    //     }
    // }
}
